...
export function AnalyticsListPage() {
  ...
  return (
    <div>
      <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
            Select a poll to view analytics dashboard.
          </p>
        </div>
      ...
      {loading ? (
        <Card className="p-6">
          <p className="text-sm text-neutral-500 dark:text-neutral-400">Loading polls...</p>
        </Card>
      ) : null}
      ...
      <div className="grid gap-3">
        {polls.map((poll) => {
          ...
          return (
            <Link to={`/dashboard/analytics/${poll.id}`} key={poll.id} className="block">
              <Card className="p-5 transition hover:-translate-y-0.5 cursor-pointer">
